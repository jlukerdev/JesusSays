...
class BibleOfflineStore {
  ...
  // chaptersMap: { [bookAbbr]: number[] } — maps each book to the specific chapter
  // numbers to download. Callers control which chapters are fetched (e.g. full NT
  // vs. catalog-only subset defined by CATALOG_CHAPTERS).
  async downloadBooks(translation, chaptersMap, fetchRaw, onProgress) {
    const total = Object.values(chaptersMap).reduce((sum, chs) => sum + chs.length, 0)
    let done = 0

    for (const abbr of NT_BOOK_ABBR_ORDER) {
      const chapterNums = chaptersMap[abbr]
